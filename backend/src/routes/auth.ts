...
export function hashPassword(password: string): string {
  return crypto
    .pbkdf2Sync(password, HASH_SALT, 100_000, 64, "sha512")
    .toString("hex");
}
...
function detectAttack(
  ...
  const isAttack =
    uResult.detectionMethod === "regex" ||
    uResult.detectionMethod === "hybrid" ||
    pResult.detectionMethod === "regex" ||
    pResult.detectionMethod === "hybrid";
  return {
    isAttack,
...
setInterval(
  () => {
    ...
    for (const [key, entry] of loginAttempts.entries()) {
      const cutoff = entry.lockedUntil
        ? entry.lockedUntil + RATE_WINDOW_MS
        : entry.firstAttempt + RATE_WINDOW_MS;
